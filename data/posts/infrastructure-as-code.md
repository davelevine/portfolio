...
I settled on GitHub Actions, with one twist. Instead of running everything on GitHub-hosted runners, I provisioned a Raspberry Pi as a self-hosted runner. The Pi sits inside my network, on Tailscale, and has line of sight to all of my hosts. That means a workflow can SSH directly into Xenlab or my Hetzner VPS without me exposing anything externally or punching holes through firewalls.

The shape is simple to describe and was decidedly not simple to get right. A push to `main` triggers the relevant workflow based on what changed. If a Docker Compose file changed, the deploy-compose workflow runs the corresponding Ansible role on the affected host. If a Terraform file changed, OpenTofu plans and applies. There's also a weekly package-updates workflow that runs `apt` upgrades across every host and reports drift back to a Healthchecks.io endpoint, which then pushes me a notification via Ntfy if anything looks off.

Getting all of that to be reliable took a while. I had a stretch where playbooks would intermittently fail because Bitwarden Secrets Manager was being hit too many times in parallel from a single run, and the Ansible run would race against itself. The fix was to serialize host execution in the site playbook so that BWS lookups didn't pile up on top of each other. There was a separate, longer-running issue where compose files on hosts would slowly drift out of sync with the repo if a deploy was interrupted partway through, and reconciliation didn't always notice. That ended up being a multi-PR effort to detect drift, self-heal it, pre-resolve sudo passwords once per run instead of per-task, and fix a callback plugin that was reporting "success" for tasks that had actually skipped. None of that is glamorous. All of it had to be done before I could trust the system enough to walk away from it.

The first time I watched a Renovate PR open, get auto-merged after CI passed, and then watched the deploy-compose workflow roll out the new image on its own without me touching anything, it felt a little surreal. I'd been doing all of that by hand for years. It also broke a few times before it stopped breaking, and I had to resist the urge to disable the automation every time it did.
...
The migration itself was painful. Every secret had to be moved into the vault, every reference had to be updated, and every old copy had to be tracked down and removed. I was finding stragglers for weeks. There was no clean cutover moment, just a slow process of grepping for anything that looked like a secret and following the trail. The result is worth it, but I wouldn't pretend it was a quick afternoon of work.

## Monitoring

A homelab without monitoring is just a collection of things you'll find out are broken when you try to use them. Before this rebuild, I was using [Uptime Kuma](https://uptime.kuma.pet/) for health checks and a patchwork of other things for everything else. Uptime Kuma was fine, but I wanted something configured in code rather than clicked into a UI, and I wanted finer control over what went where.

I ended up consolidating around four pieces. [Homepage](https://gethomepage.dev) is my dashboard, and it's the first thing I see when I open a browser tab. [Gatus](https://github.com/TwiN/gatus) replaced Uptime Kuma for actual health checking, and because it's configured in YAML, the entire thing lives in the repo alongside everything else. [Beszel](https://beszel.dev/) handles host-level metrics. [Ntfy](https://ntfy.sh) is the pipe through which everything reaches my phone.

[Healthchecks.io](https://healthchecks.io/) sits alongside all of that as my dead-man's-switch monitor for systemd timers and cron jobs. If a backup doesn't check in on time, I know about it. If a package-updates run skips a host, I know about it. The mental load of "I hope that thing ran" is gone, and that alone has been worth the effort.

The reverse proxy story is similar. I'd been on [Nginx Proxy Manager](https://nginxproxymanager.com/) for years, mostly out of inertia. As part of all of this, I migrated to [Caddy](https://caddyserver.com/) so the configuration could live in the repo. I also swapped out [Authentik](https://goauthentik.io/) for [oauth2-proxy](https://oauth2-proxy.github.io/oauth2-proxy/) plus [Pocket ID](https://pocket-id.org/), because Authentik was more service than I needed and I wanted something I could reason about end to end. Each of those migrations took a weekend or more, and each one had its own set of gotchas that I won't bore you with. The cumulative effect is that nothing in the path between the public internet and my services is configured by hand anymore.

## Where Things Stand

Once the foundation was in place, the homelab stopped feeling like a project and started feeling like infrastructure. The pace of changes has actually picked up rather than slowed down, which I think is the real test of whether the GitOps approach was worth it. A few of the things that have happened since I first started drafting this post:
...
- **A multi-week Postgres detour for Journalistic.** I run a small open-source project called [Journalistic](https://github.com/davelevine/journalistic), and at one point I decided to move its storage from SQLite plus [Litestream](https://litestream.io/) over to PostgreSQL with point-in-time recovery. The plan was to use restic for `pg_dump` snapshots, then [WAL-G](https://github.com/wal-g/wal-g) for WAL archiving to R2. WAL-G didn't fit cleanly, so I swapped to [pgBackRest](https://pgbackrest.org/). pgBackRest also wasn't the right shape, so I tried barman-cloud via the [CNPG](https://cloudnative-pg.io/) image with gzip-compressed WAL and base backups to R2. By the time I got it all working, I had to step back and admit that the operational surface area was way more than the project actually needed. I ripped the whole thing out and reverted to SQLite plus Litestream. That detour cost me weeks. I learned a lot about WAL archiving in the process, but if I'm being honest, it was a lesson in knowing when to stop.
- **Retiring MinIO in favor of VersityGW.** MinIO had been my on-prem S3 layer for years, but the [upstream repo](https://github.com/minio/minio) is now flagged as no longer maintained, the community edition has been pulled back to source-only with no binary releases, and everyone is being funneled toward [AIStor](https://min.io/product/aistor), their commercial offering. I'd already been on a community fork (firstfinger, then later pgsty) to keep up, and at some point that started feeling like a treadmill that ended somewhere I didn't want to go. I piloted [VersityGW](https://github.com/versity/versitygw) alongside MinIO on Xenlab, and once it was stable I phased the decommission into five PRs so each step was independently revertable: remove the MinIO service from Xenlab, swap the Caddy block and DNS, replace the Gatus check, swap the Homepage tile, and finally drop the SMB mount that no longer had a reason to exist. MinIO is fully retired now. The pilot itself was not smooth. I had to set explicit CORS allow-origins for the WebGUI, fix the healthcheck to use TCP instead of HTTP, move the backend off SMB to local disk because of file locking weirdness, advertise the external gateway URL correctly to the WebUI, and resolve the Sharrr IAM credentials directly from Bitwarden when the indirect path didn't work. Each of those was a small fix, and collectively they were the kind of grind that doesn't show up in a feature list.
...
If I were starting over, I'd be more disciplined about doing one thing at a time. There were stretches where I had Ansible refactors, OpenTofu imports, secrets migration, and a Caddy cutover all in flight at once, and when something broke it was much harder than it needed to be to figure out which moving part was responsible. The repo has plenty of evidence of me untangling something I'd accidentally tangled up myself.

I'd also be more skeptical of my own enthusiasm. The Postgres PITR detour is the obvious example, but it's not the only one. Several times during this rebuild I followed a thread for longer than I should have because I'd already invested time and didn't want to admit that the thing I was building wasn't actually the thing I needed. The willingness to delete a week of work and go back to the simpler answer is a skill, and I'm still building it.

## Where to Go From Here

The next stretch of work is more about polish than new construction. I want to get to the point where every host can be reprovisioned from scratch with no human in the loop beyond kicking off the first playbook. I'm close, but not there yet. There are still a handful of bootstrap steps that require me to do something by hand, and each one is a small failure of the larger goal.

Beyond that, I'd like to extend the GitOps approach to a couple of corners of my setup that I haven't touched yet. My pfSense configuration is still managed through its web UI. My UniFi controller config is managed by the controller itself, which is fine until the day it isn't. Both of those are candidates for being pulled into the same model as everything else, although I haven't yet decided whether the juice is worth the squeeze.

The bigger thing, and the part I keep coming back to, is that the time investment up front buys back time on the other end in a way that compounds. I spend less time fighting my homelab now than I did a year ago, even though it does more, because the friction of making any individual change is so much lower. That doesn't mean I never spend a Saturday chasing down a sudo password race condition or untangling a bind mount that should have been straightforward. It means that when I do, the work is captured in code at the end of it, and I don't have to do it again.
